...
if [ -f "${REPORT_PATH}" ]; then
  ...
  SCREENSHOTS_CELL="$(node -e "const fs=require('fs'); const r=JSON.parse(fs.readFileSync(process.argv[1],'utf8')); const shots=r.screenshots||[]; if(!shots.length){process.stdout.write('None captured'); process.exit(0);} const isLaunch=(s)=>/^\\d+-after-open\\.(png|jpe?g)\$/i.test(String(s.fileName||'')); const isPassport=(s)=>/-passport\\.(png|jpe?g)\$/i.test(String(s.fileName||'')); const evidence=shots.filter((s)=>!isLaunch(s)); const ordered=[...evidence.filter(isPassport),...evidence.filter((s)=>!isPassport(s))]; const list=ordered.length?ordered:shots; process.stdout.write(list.slice(0,4).map((s)=>s.label||s.fileName).join('<br>'));" "${REPORT_PATH}")"
  SCREENSHOTS_PREVIEW="Screenshot previews will be published after QA completes."
